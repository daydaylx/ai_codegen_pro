...
class MainWindow(QMainWindow):
    ...
    def post_init_setup(self):
        """Führt blockierende Aufgaben aus, nachdem die GUI sichtbar ist."""
    ...
    def load_stylesheet(self):
        ...
        try:
            ...
            print(f"Warnung: Stylesheet {style_path} nicht gefunden.")

    def load_models(self):
        api_key = os.environ.get("AICODEGEN_API_KEY")
        if not api_key: return

        self.model_box.clear(); self.model_box.addItem("Lade Modelle...")
    ...
    def on_model_load_error(self, error_msg):
        self.model_box.clear(); self.model_box.addItem("Fehler")
    ...
    def run_generation(self):
        ...
        if not all([user_prompt, model_id, api_key]):
            QMessageBox.warning(self, "Eingabe fehlt", "Bitte Prompt ausfüllen und Modell auswählen."); return

        self.send_btn.setEnabled(False); self.output_area.clear()
        full_system_prompt = self.sys_prompt.text_edit.toPlainText().strip()
        regel_prompt = self.regel_prompt.text_edit.toPlainText().strip()
        if regel_prompt: full_system_prompt += f"\n\n--- REGELN ---\n{regel_prompt}"
    ...
    def append_chunk(self, chunk):
        self.output_area.moveCursor(QTextCursor.End); self.output_area.insertPlainText(chunk)
    ...
    def clear_all(self):
        self.sys_prompt.text_edit.clear(); self.regel_prompt.text_edit.clear()
        self.norm_prompt.text_edit.clear(); self.output_area.clear()

    def prompt_for_api_key(self):
        key, ok = QInputDialog.getText(self, "API-Key benötigt", "OpenRouter API-Key:", text=self.settings.get("api_key", ""))
        if ok and key:
